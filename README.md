...
```bash
uv run python scripts/prepare_release.py 0.1.1
...
Use `--no-commit --no-tag` to update files and run the checks without creating
the release commit or tag, and `--dry-run` to preview the plan:

```bash
uv run python scripts/prepare_release.py 0.1.1 --no-commit --no-tag
```
