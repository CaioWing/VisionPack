...
from visionpack.core.models import Annotation, Asset, BBox, ObjectAnnotation, Polygon, utc_now
...
def export_yolo(
    ...
    ``seg`` writes YOLO-seg polygon labels instead of boxes (defaults to on for
    segmentation projects). Polygons export one line per ring; box-only objects
    are written as their four corners so mixed datasets stay trainable.
    """
    output = output.resolve()
    if seg is None:
        seg = project.manifest.task == "segmentation"
        ...
        if annotation:
            for obj in annotation.objects:
                ...
                if bbox is None:  # whole-image label (classification): no box to write
                    continue
                if seg:
                    lines.extend(_seg_lines(obj, bbox, class_index[obj.class_id], asset.width, asset.height))
                else:
...
def _seg_lines(obj: ObjectAnnotation, bbox: BBox, class_index: int, width: int, height: int) -> list[str]:
    """YOLO-seg lines for one object: each polygon ring on its own line; a plain
    box degrades to its four corners so it stays a valid polygon label."""
    if isinstance(obj.geometry, Polygon):
        rings = [ring for ring in obj.geometry.rings if len(ring) >= 6]
    else:
        rings = [[bbox.x, bbox.y, bbox.x + bbox.width, bbox.y, bbox.x + bbox.width, bbox.y + bbox.height, bbox.x, bbox.y + bbox.height]]
    lines = []
    for ring in rings:
        normalized = (
            _fmt(min(max(value / (width if i % 2 == 0 else height), 0.0), 1.0)) for i, value in enumerate(ring)
        )
...
def parse_yolo_label_text(
    ...
    Both label dialects are accepted per line: detection (``class cx cy w h``,
    exactly 5 values) and YOLO-seg (``class x1 y1 x2 y2 ...``, class plus at
    least three normalized vertex pairs). ``origin`` is only used in error
    messages (a path or URI), so this works the same whether the text came from
    disk or a remote object.
    """
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if len(values) == 4:
            ...
            geometry: BBox | Polygon = BBox(
                x=x_center * width - abs_width / 2, y=y_center * height - abs_height / 2, width=abs_width, height=abs_height
            )
        elif len(values) >= 6 and len(values) % 2 == 0:
            ring = [values[i] * (width if i % 2 == 0 else height) for i in range(len(values))]
            geometry = Polygon(rings=[ring])
        else:
            raise FormatError(
                f"Invalid YOLO label at {origin}:{line_number}. "
                "Expected `class x_center y_center width height` or a YOLO-seg polygon `class x1 y1 x2 y2 x3 y3 ...`."
            )
        objects.append(ObjectAnnotation(class_id=class_id, geometry=geometry))
    return objects
